...
import 'package:ppac/Response/RtiResponse.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../Home/HomeDashboard.dart';

class RtiDataListnew extends StatefulWidget {
  const RtiDataListnew({Key? key}) : super(key: key);

  @override
  RtiDataListnewState createState() => RtiDataListnewState();
}

class RtiDataListnewState extends State<RtiDataListnew> {
  String? htmlContent = "";
  String? menuname;
  ...
  Future<List<RtiData>> fetchRtiData() async {
    try {
      Map data = {
      ...
      if (response.statusCode == 200) {
        for (var objrti in decoded["RtiData"]) {
          arrrti.add(RtiData(
              id: objrti["id"],
              title: objrti["title"],
              fileUrl: objrti["file_url"],
              imagePath: objrti["image_path"],
              externalUrl: objrti["external_url"],
              content: objrti["content"],
              createdAt: objrti["created_at"],
              updatedAt: objrti["updated_at"]));
        }

        if (arrrti.isNotEmpty && decoded["htmlcontent"] == "False") {
          print("RTI DATA:$arrrti");
        ...
        RtiContent.rtititle = decoded["Page Title"];
        ;
        if (arrrti.length < perPage) {
          perPage = arrrti.length;
        }
        items.addAll(arrrti.getRange(present, present + perPage));
  ...
  onSearch(String search) {
    setState(() {
      if (editingController.text.isEmpty) {
        ...
        items = arrrti
            .where((objrti) =>
                objrti.title.toLowerCase().contains(search.toLowerCase()) ||
                objrti.content.toLowerCase().contains(search.toLowerCase()))
            .toList();
      }
    });
  }
  ...
  void loadMore() {
    setState(() {
      if ((present + perPage) > arrrti.length) {
        items.addAll(arrrti.getRange(present, arrrti.length));
      } else {
        items.addAll(arrrti.getRange(present, present + perPage));
      }
      present = present + perPage;
  ...
  showRtiListView() {
    return ListView.builder(
        ...
        physics: const ScrollPhysics(),
        itemCount: (present <= arrrti.length) ? items.length + 1 : items.length,
                      ...
                      child: TextButton(
                        ...
                        onPressed: () {
                          setState(() {
                            if ((present + perPage) > arrrti.length) {
                              items.addAll(
                                  arrrti.getRange(present, arrrti.length));
                            } else {
                              items.addAll(
                                  arrrti.getRange(present, present + perPage));
                            }
                            present = present + perPage;
                          });
                        },
